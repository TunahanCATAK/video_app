...
bool load_frame(const char* filename, int* width_out, int* height_out, unsigned char** data_out){
    ...
    if (0 != avformat_open_input(&av_format_ctx, filename, NULL, NULL)) {
        printf("Coudln't open the video file\n");
        return false;
    }
    ...
    for (int i = 0; i < av_format_ctx->nb_streams; ++i) {
        ...
        if (av_codec_params->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            printf("Video stream index is: %d", video_stream_index);
            break;
        }
    }
    ...
    if (!av_codec_ctx){
        printf("Couldn't create AVCodedContext\n");
        return false;
    }
    ...
    if (!sws_scaler_ctx ) {
        printf("Couldn't initialize scaler\n");
        return false;
    }
